...
let mutable activeUserList = Map.empty
...
let addUser (user: Register) =
    let temp = userList.TryFind(user.UserName)
    if temp = None then
        userList <- userList.Add(user.UserName,user.Password)
        {Comment = "Successfully registered";Content=[];status=1;error=false}
    else
        {Comment = "A user with this name already exists, please login or enter a new user name";Content=[];status=1;error=true}

let loginuser (user: Login) = 
    printfn "Login request received from %s as %A" user.UserName user
    let temp = userList.TryFind(user.UserName)
    if temp = None then
        {Comment = "The user with this name does not exist. Please Register or use a different username ";Content=[];status=0;error=true}
    else
        if temp.Value.CompareTo(user.Password) = 0 then
            let temp1 = activeUserList.TryFind(user.UserName)
            if temp1 = None then
                activeUserList <- activeUserList.Add(user.UserName,true)
                {Comment = "Succesfully Logged In ";Content=[];status=2;error=false}
            else
                {Comment = "Already Logged In";Content=[];status=2;error=true}
        else
            {Comment = "Incorrect Password";Content=[];status=1;error=true}
...
let logoutuser (user:Logout) = 
    ...
    if temp = None then
        ...
        if temp1 = None then
            ...
            activeUserList <- activeUserList.Remove(user.UserName)
            {Comment = "User Logged out Successfully";Content=[];status=1;error=false}

let isUserLoggedIn username = 
    let temp = activeUserList.TryFind(username)
    if temp <> None then
        1 // Logged In
    else
...
let checkUserExsistance username =
    let temp = userList.TryFind(username)
    temp <> None
        ...
        if (checkUserExsistance follower.Following) then
            ...
            if temp = None then
                ...
                if temp.Value.Exists( fun x -> x.CompareTo(follower.UserName) = 0 ) then
                    ...
                    if temp1 <> None then
                        liveUserHandlerRef <! Following(temp1.Value,"You are now following: "+follower.Following)
                    {Comment = "Successfully Added to the Following list";Content=[];status=2;error=false}
        else
            {Comment = "Follower "+follower.Following+" does not exist";Content=[];status=2;error=true}
    elif status = 0 then
...
let addTweetToFollowers (tweet: NewTweet) = 
    let temp = followerList.TryFind(tweet.UserName)
    if temp <> None then
...
let addTweetToUser (tweet: NewTweet) =
    ...
    if status = 1 then
        ...
        {Comment = "User Doesn't Exist!!Please Register";Content=[];status=0;error=true}
...
let getMentions username = 
    let status = isUserLoggedIn username
    if status = 1 then
        let temp = mentionList.TryFind(username)
        if temp = None then
